...
# def nim_sudah_terdaftar(nim: str) -> bool:
#     conn   = get_conn()
#     cursor = conn.cursor()
#     try:
#         cursor.execute("SELECT 1 FROM user WHERE nim = %s LIMIT 1", (int(nim),))
...
# def simpan_mahasiswa(nim: str, nama: str) -> None:
#     """
#     INSERT mahasiswa baru, atau UPDATE nama saja kalau NIM sudah ada.
...
#     try:
#         cursor.execute("""
#                 INSERT INTO user (nim, nama, waktu_registrasi)
#                 VALUES (%s, %s, NOW())
...
#             """, (nim, nama))
#         conn.commit()
#         print(f"Data user tersimpan — NIM: {nim} | Nama: {nama}")
...
# def get_nama(nim: str) -> str:
#     conn   = get_conn()
#     cursor = conn.cursor()
#     try:
#         cursor.execute("SELECT nama FROM user WHERE nim = %s LIMIT 1", (int(nim),))
...
# def simpan_embedding(nim: str, vector: np.ndarray) -> None:
#     """
#     INSERT embedding baru, atau UPDATE vector kalau NIM sudah ada.
...
#         cursor.execute(
#             """
#             INSERT INTO embedding (nim, vector)
#             VALUES (%s, %s)
#             ON DUPLICATE KEY UPDATE vector = VALUES(vector)
#             """,
#             (int(nim), vector_bytes)
#         )
#         conn.commit()
#         print(f"Embedding tersimpan — NIM: {nim}")
#     except mysql.connector.Error as e:
#         conn.rollback()
...
#     Return dict {nim_str: np.ndarray} siap dipakai untuk pencocokan wajah.
#     """
#     conn   = get_conn()
#     cursor = conn.cursor()
#     try:
#         cursor.execute("SELECT nim, vector FROM embedding")
...
#     embeddings = {}
#     for nim, vector_bytes in rows:
#         vector = np.frombuffer(vector_bytes, dtype=np.float32)
#         embeddings[str(nim)] = vector
...
# def sudah_presensi_hari_ini(nim: str) -> bool:
#     conn   = get_conn()
#     cursor = conn.cursor()
#     try:
#         cursor.execute(
...
# def simpan_presensi(nim: str) -> None:
#     if sudah_presensi_hari_ini(nim):
#         return
...
#     try:
#         cursor.execute(
#             "INSERT INTO presensi (nim, waktu_presensi) VALUES (%s, NOW())",
#             (int(nim),)
#         )
#         conn.commit()
...
#         now  = datetime.now()
#         nama = get_nama(nim)
#         print(f"[PRESENSI] {now.strftime('%H:%M:%S')} | NIM: {nim} | {nama} | Hadir")
...
# def get_nim_presensi_hari_ini() -> set:
#     conn   = get_conn()
#     cursor = conn.cursor()
#     try:
#         cursor.execute(
#             """
#             SELECT nim FROM presensi
...
#     try:
#         query = """
#             SELECT a.nim, m.nama, a.waktu_presensi, a.status
#             FROM presensi a
#             JOIN user m ON a.nim = m.nim
#             WHERE 1=1
#         """
#         params = []

#         if keyword:
#             query += " AND (a.nim LIKE %s OR m.nama LIKE %s)"
#             like = f"%{keyword}%"
...
def simpan_embedding(nomor_identitas: str, vector: np.ndarray) -> None:
    ...
    try:
        ...
        cursor.execute(
            """
            INSERT INTO embedding (nomor_identitas, vector)
            VALUES (%s, %s)
            ON DUPLICATE KEY UPDATE vector = VALUES(vector)
            """,
...
def load_semua_embedding() -> dict:
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT nomor_identitas, vector FROM embedding")
        rows = cursor.fetchall()
    finally:
        cursor.close()
        conn.close()
